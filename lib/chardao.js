...
function getCharsForUserId(id, fn) {
    pool.query("SELECT * FROM chars WHERE accid = ?", [id], fn);
}
...
function getCharInventory(userId, charId, fn) {
    pool.query("SELECT * FROM chars JOIN char_inventory USING(charid) WHERE accid = ? AND charid = ? ORDER BY slot ASC", [userId, charId], fn);
}

function getAllChars(fn) {
    pool.query("select charname, IF(accounts_sessions.charid IS NULL, false, true) as loggedin from chars LEFT JOIN accounts_sessions USING(charid)", fn);
}
